  ...
  if (!userSnap.exists()) {
    ...
    if (refCode) {
      try {
        ...
        if (refSnap.exists()) {
          await updateDoc(refRef, { balance: increment(REFERRAL_BONUS), referrals: increment(1) });
          await addDoc(collection(db, "transactions"), {
            userId:      refCode,
            type:        "referral",
            amount:      REFERRAL_BONUS,
            description: `Referral Bonus — ${user.displayName || user.email}`,
            createdAt:   serverTimestamp()
          });
        }
      } catch(e) { console.log("Referrer error:", e); }
    }
    return true;
  }
  return false;
...
window.loadDashboard = async function() {
  onAuthStateChanged(auth, async (user) => {
    ...
    try {
      ...
      if (refLinkEl) refLinkEl.textContent = `liuscine.net/ref/${user.uid}`;

      // Copy referral link
      const copyBtn = document.querySelector('.ref-copy-btn');
      if (copyBtn) copyBtn.onclick = () => copyText(`liuscine.net/ref/${user.uid}`, 'Referral link copied!');

      // Share buttons
      document.querySelectorAll('.share-btn').forEach(btn => {
        btn.onclick = () => {
          const platform = btn.querySelector('i')?.className || '';
          const link = `liuscine.net/ref/${user.uid}`;
          const text = `Watch free Nollywood movies & earn money on LiusCine! Join here: ${link}`;
          if (platform.includes('whatsapp')) window.open(`https://wa.me/?text=${encodeURIComponent(text)}`);
          else if (platform.includes('instagram')) { copyText(link,'Link copied! Paste in your bio'); }
          else if (platform.includes('tiktok')) { copyText(link,'Link copied! Paste in your bio'); }
        };
      });
...
window.requestWithdrawal = async function() {
  ...
  try {
    ...
    await addDoc(collection(db,"withdrawals"), { userId:user.uid, userName:userSnap.data().name, userEmail:user.email, amount, bank, accountNumber:account, status:"pending", requestedAt:serverTimestamp() });
    await addDoc(collection(db,"transactions"), { userId:user.uid, type:"withdrawal", amount, description:`Withdrawal — ${bank}`, createdAt:serverTimestamp() });

    document.getElementById('withdrawModal').classList.remove('open');
    showToast('Withdrawal submitted! Processing within 24hrs ✅', 'success');
...
window.handleUpgrade = async function() {
  ...
  const handler = window.PaystackPop?.setup({
    ...
    currency: 'NGN',
    ref:      'LC-PREM-' + Date.now(),
    metadata: { userId: user.uid, type: 'premium' },
    callback: async function(response) {
      try {
        await updateDoc(doc(db,"users",user.uid), { premium: true });
        await addDoc(collection(db,"transactions"), { userId:user.uid, type:"premium", amount:PREMIUM_AMOUNT, description:"Premium Subscription Activated", createdAt:serverTimestamp() });
        showToast('🎉 You are now Premium! Enjoy HD downloads & zero ads.', 'success');
        setTimeout(() => window.location.reload(), 1500);
      } catch(e) { showToast('Payment received but upgrade failed. Contact support.', 'error'); }
    },
    onClose: function() { showToast('Payment cancelled', 'info'); }
  });
  if (handler) handler.openIframe();
  else showToast('Payment system loading. Try again.', 'info');
};
