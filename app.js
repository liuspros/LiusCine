...
window.handleUpgrade = async function() {
  ...
  const handler = window.PaystackPop?.setup({
    key:      PAYSTACK_KEY,
    email:    user.email,
    amount:   PREMIUM_AMOUNT * 100, // kobo
    currency: 'NGN',
    ...
    callback: async function(response) {
      try {
        ...
        await updateDoc(doc(db,"users",user.uid), { premium: true, premiumActivatedAt: serverTimestamp() });
        ...
        // 3. Credit the referrer ₦500 NOW (premium confirmed)
